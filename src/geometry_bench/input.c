...
SDL_bool geometry_handle_input(GeometryBenchState *state, BenchMetrics *metrics, BenchOverlay *overlay)
{
    SDL_Event e;
            ...
            switch (sym) {
                ...
                case BTN_X:
                    state->geometry_render_mode =
                        (state->geometry_render_mode + 1) % GEOMETRY_RENDER_MODE_MAX;
                    break;
