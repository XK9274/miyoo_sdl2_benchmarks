...
#include "geometry_bench/input.h"
...
static const char *g_geometry_mode_labels[GEOMETRY_RENDER_MODE_MAX] = {
    "Filled Faces", "Wireframe", "Vertex Points",
};

static const OverlayRowSpec g_geometry_rows[] = {
...
static const OverlayKeybind g_geometry_keybinds[] = {
...
int main(int argc, char *argv[])
{
    ...
    SDL_Window *window = SDL_CreateWindow("SDL2 Geometry (NEON) Bench",
    ...
    GeometryBenchState state;
    geometry_state_init(&state);

    const char *bench_duration_str = SDL_getenv("GEOMETRY_BENCH_DURATION_S");
    const double bench_duration_s = bench_duration_str ? SDL_atof(bench_duration_str) : 0.0;
    const char *bench_tag = SDL_getenv("GEOMETRY_BENCH_TAG");
    ...
    geometry_render_init(&state, renderer);
    ...
    if (!overlay) {
        ...
        geometry_render_cleanup(&state);
        geometry_state_destroy(&state, renderer);
    ...
    bench_overlay_configure(overlay, g_geometry_rows, (int)SDL_arraysize(g_geometry_rows),
                            g_geometry_keybinds, (int)SDL_arraysize(g_geometry_keybinds));
    if (loading_active) {
        bench_loading_step(&loading, 0.8f, "Geometry bench ready");
        bench_loading_finish(&loading);
        loading_active = SDL_FALSE;
    }

    printf("SDL2 Geometry (NEON) Bench initialised\n");
    ...
    while (running) {
        ...
        if (!geometry_handle_input(&state, &metrics, overlay)) {
            break;
        }
        ...
        geometry_render_scene(&state, renderer, &metrics, delta_seconds);
        ...
        snprintf(stress_label, sizeof(stress_label), "Stress L%d x%.1f",
                 state.stress_level, geometry_state_stress_factor(&state));
        const int mode_index = (state.geometry_render_mode >= 0) ?
            (state.geometry_render_mode % GEOMETRY_RENDER_MODE_MAX) : 0;
        char mode_label[64];
        snprintf(mode_label, sizeof(mode_label), "Geometry Mode: %s | Triangles: %d",
                 g_geometry_mode_labels[mode_index], state.geometry_triangle_count);
