#include "geometry_bench/render.h"
...
#define GEOMETRY_BENCH_PI 3.14159265358979323846f
...
/* SoA layout lets rotation/projection batch 4-wide with NEON. dir_x/y/z is
 * each vertex's parent face's outward centroid direction, applied as the
 * explosion offset before rotation. */
typedef struct {
    SDL_bool valid;
    int subdivisions;
    int triangle_count;
 ...
 * hidden whenever their target triangle faces the camera (its outer surface
 * then occludes the inner-surface target). */
typedef struct {
...
static inline float geometry_clampf(float value, float min_val, float max_val)
{
...
static inline int geometry_clampi(int value, int min_val, int max_val)
{
    if (value < min_val) return min_val;
    if (value > max_val) return max_val;
    return value;
...
static RotationTrig geometry_build_rotation_trig(const GeometryBenchState *state,
                                                 float rx,
                                                 float ry,
                                                 float rz)
{
    RotationTrig trig;
    trig.sx = geometry_state_sin_rad(state, rx);
    trig.cx = geometry_state_cos_rad(state, rx);
    trig.sy = geometry_state_sin_rad(state, ry);
    trig.cy = geometry_state_cos_rad(state, ry);
    trig.sz = geometry_state_sin_rad(state, rz);
    trig.cz = geometry_state_cos_rad(state, rz);
    return trig;
}

#if BENCH_HAS_NEON
/* Newton-Raphson refined reciprocal: ARMv7 NEON has no vdivq_f32. */
static inline float32x4_t geometry_neon_recip_f32(float32x4_t d)
{
 ...
 * a per-vertex rotate + perspective-project into a single SIMD-batched
 * sweep over SoA position arrays. */
static void geometry_rotate_project_batch(const float *vx, const float *vy, const float *vz,
                                          ...
                                          float *out_x, float *out_y)
{
    ...
    for (; i + 4 <= count; i += 4) {
        ...
        float32x4_t persp = geometry_neon_recip_f32(denom);
...
static Vector3 geometry_icos_normalize(Vector3 v)
{
...
static void geometry_subdivide_triangle(TriangleCache *cache, int *count,
                                        ...
                                        int level)
{
    ...
    geometry_subdivide_triangle(cache, count, v0, m01, m20, face_dir, r, g, b, a, level - 1);
    geometry_subdivide_triangle(cache, count, m01, v1, m12, face_dir, r, g, b, a, level - 1);
    geometry_subdivide_triangle(cache, count, m20, m12, v2, face_dir, r, g, b, a, level - 1);
    geometry_subdivide_triangle(cache, count, m01, m12, m20, face_dir, r, g, b, a, level - 1);
}

static void geometry_build_exploded_icosahedron(TriangleCache *cache, int subdivisions)
{
    int count = 0;
    ...
    for (int f = 0; f < 20 && count < MAX_TRIANGLES; f++) {
        ...
        const Vector3 face_dir = geometry_icos_normalize(centroid);

        const Uint8 face_r = (Uint8)(100 + (f * 37) % 156);
        const Uint8 face_g = (Uint8)(120 + (f * 61) % 136);
        const Uint8 face_b = (Uint8)(180 - (f * 7) % 120);

        geometry_subdivide_triangle(cache, &count, v0, v1, v2, face_dir,
...
static TriangleCache *geometry_get_cached_icosahedron(int subdivisions, int *triangle_count)
{
    const int clamped = geometry_clampi(subdivisions, 0, ICOS_SUBDIVISION_MAX);
    TriangleCache *cache = &g_triangle_cache[clamped];

    if (!cache->valid || cache->subdivisions != clamped) {
        geometry_build_exploded_icosahedron(cache, clamped);
...
static void geometry_emitter_spawn_particle(int index, const TriangleCache *cache, int triangle_count)
{
    if (triangle_count <= 0) {
        return;
    }
...
static void geometry_update_star_field(StarField *field, int count,
                                       float delta_seconds, float center_x, float center_y,
                                       const GeometryBenchState *state)
{
    int i = 0;
    ...
    for (i = 0; i < count; i++) {
        if (field->x[i] < 0 || field->x[i] > bench_logical_w() ||
            ...
            float angle = (float)rand() / RAND_MAX * 2.0f * GEOMETRY_BENCH_PI;
            float speed = 20.0f + (float)rand() / RAND_MAX * 80.0f;
            field->dx[i] = geometry_state_cos_rad(state, angle) * speed;
            field->dy[i] = geometry_state_sin_rad(state, angle) * speed;
...
static void geometry_render_star_field(SDL_Renderer *renderer, const StarField *field,
                                       int count, BenchMetrics *metrics)
{
    for (int i = 0; i < count; i++) {
        Uint8 alpha = (Uint8)(field->a[i] * geometry_clampf(field->life[i], 0.0f, 1.0f));
...
static inline SDL_bool geometry_triangle_is_front_facing(float x0, float y0, float x1, float y1, float x2, float y2)
{
...
static void geometry_compute_front_facing(const float *proj_x, const float *proj_y,
                                          int triangle_count, SDL_bool *out_front_facing)
{
    for (int i = 0; i < triangle_count; i++) {
        const int base = i * 3;
        out_front_facing[i] = geometry_triangle_is_front_facing(proj_x[base], proj_y[base],
                                                                proj_x[base + 1], proj_y[base + 1],
                                                                proj_x[base + 2], proj_y[base + 2]);
    }
}

static void geometry_render_triangles_cpu(SDL_Renderer *renderer,
...
static void geometry_render_triangle_points(SDL_Renderer *renderer,
                                            const TriangleCache *cache,
                                            const float *proj_x, const float *proj_y,
                                            const SDL_bool *front_facing,
                                            int triangle_count,
                          ...
                          double delta_seconds)
{
    ...
    const float factor = geometry_state_stress_factor(state);
    ...
    const int max_triangles = geometry_clampi((int)(base_triangles + factor * 60), base_triangles, MAX_TRIANGLES);
    state->geometry_triangle_count = max_triangles;

    const int subdivisions = geometry_clampi(
        (int)(logf((float)max_triangles / 20.0f) / logf(4.0f) + 0.5f), 0, ICOS_SUBDIVISION_MAX);
    int cached_triangle_count = 0;
    TriangleCache *cache = geometry_get_cached_icosahedron(subdivisions, &cached_triangle_count);
    const int triangle_count = geometry_clampi(cached_triangle_count, 0, max_triangles);

    const int particle_count = geometry_clampi((int)(50 + factor * 150), 50, MAX_STAR_PARTICLES);
    geometry_update_star_field(&g_star_field,
                               particle_count,
                               (float)delta_seconds,
                               center_x,
                               center_y,
    ...
    geometry_render_star_field(renderer, &g_star_field, particle_count, metrics);

    const float scale = 80.0f + 40.0f * geometry_state_sin(state, state->geometry_phase * 10.0f);
    ...
    const float explosion_distance = explosion_base +
        explosion_amplitude * geometry_state_sin(state, state->geometry_phase * 6.0f);

    RotationTrig rotation_trig = geometry_build_rotation_trig(state,
    ...
    if (subdivisions != g_emitter_cache_subdivisions) {
        for (int i = 0; i < MAX_EMITTER_PARTICLES; i++) {
            geometry_emitter_spawn_particle(i, cache, triangle_count);
        }
        g_emitter_cache_subdivisions = subdivisions;
    }

    const int emitter_particle_count = geometry_clampi((int)(20 + factor * 40), 20, MAX_EMITTER_PARTICLES);
    ...
    for (int i = 0; i < emitter_particle_count; i++) {
        ...
        for (int k = 0; k < EMITTER_TRAIL_SAMPLES; k++) {
            const float sample_progress = geometry_clampf(g_emitter.progress[i] - (float)k * EMITTER_TRAIL_STEP, 0.0f, 1.0f);
    ...
    geometry_rotate_project_batch(exploded_x, exploded_y, exploded_z, vertex_count + trail_point_count,
                                  &rotation_trig, center_x, center_y, scale, proj_x, proj_y);

    static SDL_bool front_facing[MAX_TRIANGLES];
    geometry_compute_front_facing(proj_x, proj_y, triangle_count, front_facing);

    const GeometryRenderMode render_mode =
        (GeometryRenderMode)(state->geometry_render_mode % GEOMETRY_RENDER_MODE_MAX);

    if (render_mode == GEOMETRY_RENDER_FILLED) {
        geometry_render_triangles_cpu(renderer, cache, proj_x, proj_y, front_facing, triangle_count, metrics);
    } else if (render_mode == GEOMETRY_RENDER_POINTS) {
        geometry_render_triangle_points(renderer, cache, proj_x, proj_y, front_facing, triangle_count, metrics);
