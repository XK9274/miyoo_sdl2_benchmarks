...
static const OverlayRowSpec g_memory_rows[] = {
...
int main(int argc, char *argv[])
{
    ...
    SDL_Window *window = SDL_CreateWindow("SDL2 Memory Management Bench",
    ...
    memory_render_init(&state, renderer);
    ...
    bench_overlay_configure(overlay, g_memory_rows, (int)SDL_arraysize(g_memory_rows),
                            g_memory_keybinds, (int)SDL_arraysize(g_memory_keybinds));
    if (loading_active) {
        bench_loading_step(&loading, 0.8f, "Memory management bench ready");
    ...
    while (running) {
        ...
        if (!memory_handle_input(&state, &metrics, overlay)) {
            break;
        }
