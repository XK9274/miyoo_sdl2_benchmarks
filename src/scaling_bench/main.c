...
int main(int argc, char *argv[])
{
    ...
    const char *bench_duration_str = SDL_getenv("SCALING_BENCH_DURATION_S");
    const double bench_duration_s = bench_duration_str ? SDL_atof(bench_duration_str) : 0.0;
    const char *bench_tag = SDL_getenv("SCALING_BENCH_TAG");
    ...
    while (running) {
        ...
        if (bench_tag && metrics.accumulated_frame_time_ms >= next_bench_log_ms) {
            printf("[BENCH] tag=%s elapsed_s=%.1f frame=%llu fps=%.2f avg_fps=%.2f "
                   "min_fps=%.2f max_fps=%.2f frame_ms=%.3f scaling_ops=%llu scaling_ms=%.3f\n",
                   ...
                   (unsigned long long)metrics.scaling_operations, metrics.scaling_overhead_ms);
            fflush(stdout);
