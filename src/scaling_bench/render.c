...
static inline int scaling_clampi(int value, int min_val, int max_val)
{
    if (value < min_val) return min_val;
    if (value > max_val) return max_val;
    return value;
...
static void scaling_draw_test_content(const ScalingBenchState *state,
                                      ...
                                      BenchMetrics *metrics)
{
    ...
    for (int i = 0; i < bar_count; i++) {
        ...
        float wave = scaling_state_sin_rad(state, phase + t * SCALING_BENCH_PI * 4.0f) * 0.5f + 0.5f;
    ...
    for (int i = 0; i < shape_count; i++) {
        float angle = phase + (float)i * SCALING_BENCH_PI * 2.0f / (float)shape_count;
        float radius = (float)SDL_min(width, height) * 0.2f;
        int cx = (int)((float)width * 0.5f + scaling_state_cos_rad(state, angle) * radius * 0.5f);
        int cy = (int)((float)height * 0.5f + scaling_state_sin_rad(state, angle) * radius * 0.5f);
        int size = (int)(8.0f + 8.0f * scaling_state_sin_rad(state, phase * 2.0f + (float)i));
...
static void scaling_test_logical_scaling(const ScalingBenchState *state,
                                         SDL_Renderer *renderer,
                                         int target_width,
                                         int target_height,
                                         float phase,
                                         BenchMetrics *metrics)
{
    ...
    scaling_draw_test_content(state, renderer, target_width, target_height, phase, metrics);
...
static void scaling_test_viewport_scaling(const ScalingBenchState *state,
                                          SDL_Renderer *renderer,
                                          int target_width,
                                          int target_height,
                                          float center_x,
                                          ...
                                          BenchMetrics *metrics)
{
    ...
    scaling_draw_test_content(state, renderer, viewport.w, viewport.h, phase, metrics);
...
void scaling_render_init(ScalingBenchState *state, SDL_Renderer *renderer)
{
    ...
    state->scaling_target_count = scaling_clampi(scaling_resolution_count, 1, MAX_SCALING_TARGETS);
                         ...
                         double delta_seconds)
{
    ...
    const int tests_per_frame = scaling_clampi((int)(1 + factor * 2), 1, 3);
    ...
    for (int test = 0; test < tests_per_frame; test++) {
        ...
        switch (current_mode) {
            case SCALING_MODE_LOGICAL:
                scaling_test_logical_scaling(state, renderer, test_width, test_height, test_phase, metrics);
                break;

            case SCALING_MODE_VIEWPORT:
                scaling_test_viewport_scaling(state, renderer, test_width, test_height,
                                              center_x, center_y, test_phase, metrics);
                break;

            case SCALING_MODE_TEXTURE_TARGET:
                scaling_test_texture_target_scaling(renderer, state, test_width, test_height,
                                                    center_x, center_y, test_phase, metrics);
                break;
        }
    }
