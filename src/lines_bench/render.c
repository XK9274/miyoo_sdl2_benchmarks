...
#define LINES_BENCH_TWO_PI 6.28318530717958647692f
#define LINES_BENCH_MAX_GRID_N 12
#define LINES_BENCH_MAX_COLUMNS (LINES_BENCH_MAX_GRID_N * LINES_BENCH_MAX_GRID_N)
#define LINES_BENCH_MAX_COLUMN_HEIGHT 24
/* Worst case: every cube exposes all 4 sides plus one top face per column,
 * sized for grid_n=12, max column height=24. */
#define LINES_BENCH_MAX_EXPOSED_FACES 12288
#define LINES_BENCH_MAX_ANOMALIES LINES_BENCH_MAX_GRID_N
...
static const LinesFaceDef g_lines_face_defs[LINES_FACE_AXIS_COUNT] = {
    [LINES_FACE_TOP] = {{4, 5, 6, 7}, 1.15f, 0.5f, 0.5f, 1.0f},
    [LINES_FACE_PX]  = {{1, 2, 6, 5}, 0.85f, 1.0f, 0.5f, 0.5f},
    [LINES_FACE_NX]  = {{0, 3, 7, 4}, 0.55f, 0.0f, 0.5f, 0.5f},
    [LINES_FACE_PY]  = {{3, 2, 6, 7}, 0.85f, 0.5f, 1.0f, 0.5f},
    [LINES_FACE_NY]  = {{0, 1, 5, 4}, 0.55f, 0.5f, 0.0f, 0.5f},
};
...
static int g_lines_heights[LINES_BENCH_MAX_COLUMNS];
static Uint8 g_lines_colors_r[LINES_BENCH_MAX_COLUMNS][LINES_BENCH_MAX_COLUMN_HEIGHT];
static Uint8 g_lines_colors_g[LINES_BENCH_MAX_COLUMNS][LINES_BENCH_MAX_COLUMN_HEIGHT];
static Uint8 g_lines_colors_b[LINES_BENCH_MAX_COLUMNS][LINES_BENCH_MAX_COLUMN_HEIGHT];

static LinesFace g_lines_faces[LINES_BENCH_MAX_EXPOSED_FACES];
static int g_lines_face_count;

static LinesAnomaly g_lines_anomalies[LINES_BENCH_MAX_ANOMALIES];
static int g_lines_anomaly_count;

static LinesDrawItem g_lines_draw_items[LINES_BENCH_MAX_EXPOSED_FACES + LINES_BENCH_MAX_ANOMALIES];
...
static inline int lines_clampi(int value, int min_val, int max_val)
{
...
static inline float lines_clampf(float value, float min_val, float max_val)
{
    if (value < min_val) return min_val;
    if (value > max_val) return max_val;
    return value;
...
static Uint8 lines_hsl_channel(float l, float a, float n, float h)
{
    const float k = fmodf(n + h / 30.0f, 12.0f);
    float t = fminf(k - 3.0f, 9.0f - k);
    t = fminf(t, 1.0f);
    t = fmaxf(t, -1.0f);
    const float channel = lines_clampf(l - a * t, 0.0f, 1.0f);
    return (Uint8)(channel * 255.0f + 0.5f);
}

static void lines_hsl_to_rgb(float h, float s_pct, float l_pct, Uint8 *out_r, Uint8 *out_g, Uint8 *out_b)
{
    const float s = s_pct / 100.0f;
    const float l = l_pct / 100.0f;
    const float a = s * fminf(l, 1.0f - l);
    *out_r = lines_hsl_channel(l, a, 0.0f, h);
    *out_g = lines_hsl_channel(l, a, 8.0f, h);
    *out_b = lines_hsl_channel(l, a, 4.0f, h);
}

static inline int lines_height_at(const int *heights, int grid_n, int gx, int gy)
{
...
static void lines_add_face(float wx, float wy, int gz, LinesFaceAxis axis, Uint8 r, Uint8 g, Uint8 b)
{
    if (g_lines_face_count >= LINES_BENCH_MAX_EXPOSED_FACES) {
        return;
    }
    const LinesFaceDef *def = &g_lines_face_defs[axis];
    LinesFace *face = &g_lines_faces[g_lines_face_count++];
    face->wx = wx;
    face->wy = wy;
    face->gz = (float)gz;
    face->axis = axis;
...
static float lines_randf(void)
{
    return (float)rand() / (float)RAND_MAX;
}

static void lines_build_grid(int grid_n, float avg_height)
{
    g_lines_current_grid_n = grid_n;
    for (int i = 0; i < LINES_BENCH_MAX_COLUMNS; i++) {
        g_lines_heights[i] = 0;
    }
    ...
    for (int gx = 0; gx < grid_n; gx++) {
        for (int gy = 0; gy < grid_n; gy++) {
            ...
            const float jitter = (lines_randf() - 0.5f) * avg_height * 1.3f;
            const int height = lines_clampi((int)(avg_height + jitter + 0.5f), 1, LINES_BENCH_MAX_COLUMN_HEIGHT);
            g_lines_heights[col] = height;
            for (int level = 0; level < height; level++) {
                Uint8 r, g, b;
                lines_hsl_to_rgb(lines_randf() * 360.0f,
                                 55.0f + lines_randf() * 20.0f,
                                 52.0f + lines_randf() * 14.0f,
                                 &r, &g, &b);
    ...
    for (int gx = 0; gx < grid_n; gx++) {
        for (int gy = 0; gy < grid_n; gy++) {
            ...
            for (int level = 0; level < height; level++) {
                ...
                if (level + 1 >= height) {
                    lines_add_face(wx, wy, level, LINES_FACE_TOP, r, g, b);
                }
                if (lines_height_at(g_lines_heights, grid_n, gx + 1, gy) <= level) {
                    lines_add_face(wx, wy, level, LINES_FACE_PX, r, g, b);
                }
                if (lines_height_at(g_lines_heights, grid_n, gx - 1, gy) <= level) {
                    lines_add_face(wx, wy, level, LINES_FACE_NX, r, g, b);
                }
                if (lines_height_at(g_lines_heights, grid_n, gx, gy + 1) <= level) {
                    lines_add_face(wx, wy, level, LINES_FACE_PY, r, g, b);
                }
                if (lines_height_at(g_lines_heights, grid_n, gx, gy - 1) <= level) {
                    lines_add_face(wx, wy, level, LINES_FACE_NY, r, g, b);
                }
            }
        }
    }
...
static void lines_build_anomalies(int grid_n)
{
    g_lines_anomaly_count = 0;

    for (int gy = 0; gy < grid_n && g_lines_anomaly_count < LINES_BENCH_MAX_ANOMALIES; gy++) {
        const int gx = rand() % grid_n;
        const int col = gx * grid_n + gy;
        const int height = g_lines_heights[col];
        const float offset = (float)(grid_n - 1) * 0.5f;

        LinesAnomaly *anomaly = &g_lines_anomalies[g_lines_anomaly_count++];
        anomaly->wx = (float)gx - offset;
        anomaly->wy = (float)gy - offset;
        anomaly->base_z = (float)height;
        anomaly->float_amp = 1.1f + lines_randf() * 0.9f;
        anomaly->float_speed = 0.4f + lines_randf() * 0.4f;
        anomaly->phase = lines_randf() * LINES_BENCH_TWO_PI;
        anomaly->drift_r = 0.12f + lines_randf() * 0.14f;
        anomaly->shape_variant = gy % 3;
    }
}

static inline void lines_iso_project(float gx, float gy, float gz, float sin_r, float cos_r,
...
static inline float lines_depth(float gx, float gy, float gz, float sin_r, float cos_r)
{
    const float rx = gx * cos_r - gy * sin_r;
    const float ry = gx * sin_r + gy * cos_r;
    return rx + ry + gz;
...
static inline SDL_bool lines_face_is_backface(LinesFaceAxis axis, float a, float b)
{
    switch (axis) {
        case LINES_FACE_PX: return a <= 0.0f;
        case LINES_FACE_NX: return a >= 0.0f;
        case LINES_FACE_PY: return b <= 0.0f;
        case LINES_FACE_NY: return b >= 0.0f;
        default: return SDL_FALSE;
    }
}

static int lines_draw_item_cmp(const void *a, const void *b)
{
    const LinesDrawItem *ia = (const LinesDrawItem *)a;
    const LinesDrawItem *ib = (const LinesDrawItem *)b;
    if (ia->depth < ib->depth) return -1;
    if (ia->depth > ib->depth) return 1;
    return 0;
}

static void lines_draw_face(SDL_Renderer *renderer, const LinesFace *face,
                            float origin_x, float origin_y, float px_size,
                            float sin_r, float cos_r, SDL_bool wireframe, BenchMetrics *metrics)
{
    const LinesFaceDef *def = &g_lines_face_defs[face->axis];
    float screen_x[4], screen_y[4];

    for (int k = 0; k < 4; k++) {
        const float *corner = g_lines_cube_corners[def->corner_idx[k]];
        float px, py;
        lines_iso_project(face->wx + corner[0], face->wy + corner[1], face->gz + corner[2],
                          sin_r, cos_r, &px, &py);
        screen_x[k] = origin_x + px * px_size;
        screen_y[k] = origin_y + py * px_size;
    }

    const Uint8 r = (Uint8)lines_clampf((float)face->r * def->shade, 0.0f, 255.0f);
    const Uint8 g = (Uint8)lines_clampf((float)face->g * def->shade, 0.0f, 255.0f);
    const Uint8 b = (Uint8)lines_clampf((float)face->b * def->shade, 0.0f, 255.0f);
...
static void lines_draw_anomaly(SDL_Renderer *renderer, const LinesAnomaly *anomaly,
                        ...
                        double delta_seconds)
{
    ...
    const float factor = lines_state_stress_factor(state);
    ...
    const int grid_n = lines_clampi((int)lroundf(2.0f + (float)state->stress_level), 3, LINES_BENCH_MAX_GRID_N);
    if (grid_n != state->lines_grid_n) {
        const float avg_height = 1.0f + (float)state->stress_level * 1.1f;
        lines_build_grid(grid_n, avg_height);
        lines_build_anomalies(grid_n);
    ...
    for (int i = 0; i < g_lines_face_count; i++) {
        LinesFace *face = &g_lines_faces[i];
        if (apply_backface_cull && lines_face_is_backface(face->axis, cull_a, cull_b)) {
            continue;
        }
        face->depth = lines_depth(face->cx, face->cy, face->cz, sin_r, cos_r);
    ...
    if (state->lines_anomalies_visible) {
        for (int i = 0; i < g_lines_anomaly_count; i++) {
            LinesAnomaly *anomaly = &g_lines_anomalies[i];
            ...
            lines_iso_project(anomaly->wx + 0.5f, anomaly->wy + 0.5f, anomaly->base_z, sin_r, cos_r,
                              &anchor_x, &anchor_y);
            lines_iso_project(drift_wx + 0.5f, drift_wy + 0.5f, float_z, sin_r, cos_r, &pos_x, &pos_y);
            ...
            anomaly->depth = lines_depth(drift_wx + 0.5f, drift_wy + 0.5f, float_z, sin_r, cos_r);
    ...
    qsort(g_lines_draw_items, (size_t)item_count, sizeof(LinesDrawItem), lines_draw_item_cmp);
    ...
    for (int i = 0; i < item_count; i++) {
        const LinesDrawItem *item = &g_lines_draw_items[i];
        if (item->is_anomaly) {
            const LinesAnomaly *anomaly = &g_lines_anomalies[item->index];
            const float bob = 0.5f + 0.5f * sinf(state->lines_phase * anomaly->float_speed + anomaly->phase);
            lines_draw_anomaly(renderer, anomaly, anomaly_colors[anomaly->shape_variant], bob, metrics);
        } else {
            lines_draw_face(renderer, &g_lines_faces[item->index], origin_x, origin_y, px_size,
