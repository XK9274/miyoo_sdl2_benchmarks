...
static const OverlayRowSpec g_lines_rows[] = {
...
static const OverlayKeybind g_lines_keybinds[] = {
    {"SELECT", "Toggle overlay"},
    {"MENU", "Reset metrics"},
    {"B", "Adjust stress level"},
    {"X", "Toggle anomalies"},
    {"Y", "Toggle wireframe"},
    {"L1", "Toggle backface cull"},
};
...
int main(int argc, char *argv[])
{
    ...
    SDL_Window *window = SDL_CreateWindow("SDL2 Line & Shape Bench",
    ...
    LinesBenchState state;
    lines_state_init(&state);

    const char *bench_duration_str = SDL_getenv("LINES_BENCH_DURATION_S");
    const double bench_duration_s = bench_duration_str ? SDL_atof(bench_duration_str) : 0.0;
    const char *bench_tag = SDL_getenv("LINES_BENCH_TAG");
    ...
    lines_render_init(&state, renderer);
    ...
    if (!overlay) {
        ...
        lines_render_cleanup(&state);
        lines_state_destroy(&state, renderer);
    ...
    bench_overlay_configure(overlay, g_lines_rows, (int)SDL_arraysize(g_lines_rows),
                            g_lines_keybinds, (int)SDL_arraysize(g_lines_keybinds));
    if (loading_active) {
        bench_loading_step(&loading, 0.8f, "Line & shape bench ready");
        bench_loading_finish(&loading);
        loading_active = SDL_FALSE;
    }

    printf("SDL2 Line & Shape Bench initialised\n");
    ...
    while (running) {
        ...
        if (!lines_handle_input(&state, &metrics, overlay)) {
            break;
        }
        ...
        lines_render_scene(&state, renderer, &metrics, delta_seconds);
        ...
        snprintf(stress_label, sizeof(stress_label), "Stress L%d x%.1f",
                 state.stress_level, lines_state_stress_factor(&state));
        char mode_label[128];
        snprintf(mode_label, sizeof(mode_label),
                 "Cubes: %d | Anomalies: %d | Grid %dx%d | %s | xform %.2fms sort %.2fms draw %.2fms",
                 lines_render_cube_count(),
                 state.lines_anomalies_visible ? lines_render_anomaly_count() : 0,
        ...
        if (bench_tag && metrics.accumulated_frame_time_ms >= next_bench_log_ms) {
            printf("[BENCH] tag=%s elapsed_s=%.1f frame=%llu fps=%.2f avg_fps=%.2f "
                   "min_fps=%.2f max_fps=%.2f frame_ms=%.3f tris=%llu\n",
