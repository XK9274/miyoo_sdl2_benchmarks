...
int main(int argc, char *argv[])
{
    ...
    SDL_Window *window = SDL_CreateWindow("SDL2 Fill Rate Bench",
    ...
    bench_overlay_configure(overlay, g_fill_rows, (int)SDL_arraysize(g_fill_rows),
                            g_fill_keybinds, (int)SDL_arraysize(g_fill_keybinds));
    if (loading_active) {
        bench_loading_step(&loading, 0.8f, "Fill rate bench ready");
    ...
    while (running) {
        ...
        if (!fill_handle_input(&state, &metrics, overlay)) {
            break;
        }
        ...
        fill_render_scene(&state, renderer, &metrics, delta_seconds);
