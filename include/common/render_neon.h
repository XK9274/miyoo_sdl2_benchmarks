#ifndef COMMON_RENDER_NEON_H
#define COMMON_RENDER_NEON_H
...
static inline void bench_neon_copy_u32(uint32_t *dst, const uint32_t *src, size_t count)
{
#if BENCH_HAS_NEON
    size_t i = 0;
    for (; i + 16 <= count; i += 16) {
...
static inline void bench_neon_fill_u32(uint32_t *dst, uint32_t value, size_t count)
{
#if BENCH_HAS_NEON
    size_t i = 0;
